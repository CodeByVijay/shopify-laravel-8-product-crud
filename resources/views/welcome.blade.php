...
<body>
    <div class="container mt-4 mb-4">
        ...
        <h4>You are: <span style="color: red">{{ $shopDomain ?? Auth::user()->name }}</span></h4>
        {{-- <a href="{{ route('test') }}">Go</a> --}}
        ...
        <div class="table-responsive text-center">
            <table class="table table-bordered" id="productTable">
                ...
                <tbody>
                    <?php $i = 1; ?>
                    @forelse ($products['products'] as $row)
                        <tr>
                            <td scope="row">{{ $i++ }}</td>

                            <td><img src="{{ $row['image']['src'] }}" alt="" width="100" height="100">
                            </td>
                            <td>{{ $row['title'] }}</td>
                            <td>{!! wordwrap($row['body_html'], 30, '<br>') !!}</td>
                            <td>
                                <a href="javascript:void(0)" data-id="{{ $row['id'] }}" onclick="modelOpen(this)"
                                    class="btn btn-primary"><i class="fa fa-pencil-square"></i></a>
                                <a href="{{ route('productDelete', ['id' => $row['id']]) }}" class="btn btn-danger"
    ...
    <div class="modal fade" id="productEditModel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('editSingleProduct') }}" method="POST">
